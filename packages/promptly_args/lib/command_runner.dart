...
class CommandRunner<T> {
  ...
  CommandRunner(this.executableName, this.description, {int? usageLineLength, this.suggestionDistanceLimit = 2})
    ...
    while (commands.isNotEmpty) {
      if (argResults.command == null) {
        if (argResults.rest.isEmpty) {
          ...
          var similarCommands = _similarCommandsText(requested, commands.values);
  ...
  String _similarCommandsText(String name, Iterable<Command<T>> commands) {
    ...
    var candidates = SplayTreeSet<Command<T>>((a, b) => distances[a]! - distances[b]!);
    for (var command in commands) {
      if (command.hidden) continue;
      for (var alias in [command.name, ...command.aliases, ...command.suggestionAliases]) {
...
abstract class Command<T> {
  ...
  void set globalResults(ArgResults? value) => _globalResults = value;
